#!/usr/bin/env python
# This script converts xrdb (X11) color scheme format to vt(4) color
# scheme format
#
# Usage:
# xrdb2freebsd_vt.py path/to/xrdb/files -d /xfce4terminal/colorschemes/output
#
# Based of xrdb2terminator by Xabier Larrakoetxea
# Patched by Tobias Kortkamp
...
def main(xrdb_path, output_path=None):
    ...
    for i in filter(lambda x: xrdb_regex.match(x), os.listdir(xrdb_path)):
        ...
        for j, color in enumerate(colors):
            c = color_regex.match(color).group(2)
            if j == 7:
                c = fg_color
            output.append('kern.vt.color.{}.rgb="{}"'.format(j, c))
        output = "\n".join(output) + "\n"
        ...
        if not output_path:
            ...
            with open('{0}.conf'.format(dest), 'w+') as f:
                f.write(output)

if __name__ == "__main__":

    parser = argparse.ArgumentParser(
        description='Translate X color schemes to vt(4) format')
    parser.add_argument('xrdb_path', type=str, help='path to xrdb files')
    parser.add_argument('-d', '--destiny', type=str, dest='output_path',
                        help='path where vt(4) config files will be' +
